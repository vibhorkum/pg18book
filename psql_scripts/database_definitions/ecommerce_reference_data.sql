...
CREATE TABLE product_reference.country_of_origin (
    product_brand_id INTEGER PRIMARY KEY REFERENCES product_brand (id),
...
ALTER TABLE product_reference.product_variant_price REPLICA IDENTITY FULL;
...
CREATE OR REPLACE VIEW api.vw_product_focus AS 
    SELECT product_id, product_label, product_shortdescription, product_category_label, product_brand_label, product_variant_attributes, price, validity, current
...
CREATE OR REPLACE PROCEDURE api.update_current_price_flags()
    LANGUAGE SQL
    AS $$
    UPDATE product_reference.product_variant_price
